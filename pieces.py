...
class Piece:
	...
	def clean(self, lis):
		...
		for i in lis:
			if not i == ():
				if i[0] <= 8 and i[1] <= 8 and i[0] >= 0 and i[1] >= 1 and i not in self.get_locations_B(self.color):
					mo.append(i)
		return mo
	...
	def get_locations_B(self, color):
...
class Bishop(Piece):
	...
	def get_moves(self):
		z = lambda x, y: self.add(x, y) if not self.add(x, y) in self.get_locations() and not self.add(x,
		                                                                                               y) == () else ()
		i = 0
